...
if __name__ == '__main__':
    ...
    parser.add_argument('--gt_layer', default='3', type=int, help='graph transformer layer')
    parser.add_argument('--gt_head', default='4', type=int, help='graph transformer head')
    ...
    parser.add_argument('--tr_layer', default='3', type=int, help='transformer layer')
    parser.add_argument('--tr_head', default='8', type=int, help='transformer head')
    ...
    cross_entropy = LabelSmoothingLoss(num_classes=2, smoothing=0.1)

    Metric = ('Epoch\t\tTime\t\tAUC\t\tAUPR\t\tAccuracy\t\tPrecision\t\tRecall\t\tF1-score\t\tMcc')
    AUCs, AUPRs = [], []

    print('Dataset:', args.dataset)
    print(f'Model Config: gt_layer={args.gt_layer}, gt_head={args.gt_head}, tr_layer={args.tr_layer}, dropout={args.dropout}')
    ...
    for i in range(args.k_fold):
        ...
        for epoch in range(args.epochs):
            model.train()
            with autocast(dtype=torch.float16):  # Mixed Precision forward pass
            ...
            with torch.no_grad():
                model.eval()
                with autocast(dtype=torch.float16):  # Mixed Precision inference
        ...
        # Clean up GPU memory after fold
